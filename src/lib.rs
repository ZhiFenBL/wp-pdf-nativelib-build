...
#[unsafe(no_mangle)]
pub unsafe extern "system" fn Java_vip_zhifen_typst4j_Typst4J_render<'local>(
    mut env: JNIEnv<'local>,
    _class: JClass<'local>,
    content: JString<'local>,
    font_dir: JString<'local>,
    root_dir: JString<'local>, // <-- 1. Add the new parameter here
) -> JByteArray<'local> {
    // 1. Get input strings from Java
    let input: String = env
        .get_string(&content)
        .expect("Couldn't get java string for content!")
        .into();

    let font_dir_path: String = env
        .get_string(&font_dir)
        .expect("Couldn't get java string for fontDir!")
        .into();

    // 2. Configure font searching options.
    let mut font_options = typst_kit_options::TypstKitFontOptions::new()
        .include_system_fonts(false)
        .include_dirs([font_dir_path]);

    #[cfg(feature = "typst-kit-embed-fonts")]
    {
        font_options = font_options.include_embedded_fonts(false);
    }

    let root_dir_path: String = env
        .get_string(&root_dir)
        .expect("Couldn't get java string for rootDir!")
        .into();


    // 3. Build the Typst engine with the custom font path
    let engine = TypstEngine::builder()
        .main_file(input)
        .search_fonts_with(font_options)
        .with_package_file_resolver()
        .with_file_system_resolver(root_dir_path)
        .build();

    // 4. Compile the input string into a document.
    let document = match engine.compile::<PagedDocument>().output {
        Ok(doc) => doc,
        Err(err) => {
            let error_message = format!("Typst compilation failed: {:?}", err);
            env.throw_new("java/lang/RuntimeException", error_message).expect("Failed to throw exception");
            // Return null for the object, NOT another JNI call
            return JByteArray::from(JObject::null());
        }
    };

    // 5. Render the document to a PDF.
    let pdf_bytes = match typst_pdf::pdf(&document, &PdfOptions::default()) {
        Ok(pdf) => pdf,
        Err(err) => {
            let error_message = format!("PDF export failed: {:?}", err);
            env.throw_new("java/lang/RuntimeException", error_message).expect("Failed to throw exception");
            return env.new_byte_array(0).unwrap();
        }
    };

    // 6. Convert the resulting PDF bytes into a Java byte array.
    env.byte_array_from_slice(&pdf_bytes)
